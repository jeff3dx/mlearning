...
/*
    ...
    3. If any of the centroids moved in the last step, repeat. If nothing moved, exit.
*/
...
export default class Knn extends Component {
  ...
  initMeans = (k, extents) => {
    ...
    while (k--) {
      ...
      for (let dimensionIndex in extents) {
        mean[dimensionIndex] =
          extents[dimensionIndex][MIN_COLUMN] + Math.random() * extents[dimensionIndex][RANGE_COLUMN];
      }
      means.push(mean);
    }
    ...
    for (let dataIndex in data) {
      ...
      for (let meanIndex in means) {
        ...
        for (let dimensionIndex in dataPoint) {
          let difference = dataPoint[dimensionIndex] - meanPoint[dimensionIndex];
      ...
      assignments[dataIndex] = distances.indexOf(Math.min.apply(null, distances));
    }
  ...
  renderMeans = (means, scales, iteration) => {
    const update = d3.select('.means-plot-area').selectAll('.mean-point')
      .data(means);
      ...
      update.transition().duration(MEANS_ANIM_DURATION).delay(ITERATION_DELAY * iteration)
        .attr('cx', d => scales.x(d[0]))
        .attr('cy', d => scales.y(d[1]));
  }
  ...
  removeAssignmentLines() {
    d3.select('.assignments-plot-area').selectAll('.assignment-line').remove();
  ...
  renderAssignmentLines = (assignments, trainingData, means, iteration, retain) => {
    const { scales } = this.state;
      ...
      const update = d3.select('.assignments-plot-area').selectAll('.assignment-line i' + iteration)
        .data(assignments);
      ...
      update.enter().append('line')
        ...
        .transition().duration(1).delay((ITERATION_DELAY * iteration) + MEANS_ANIM_DURATION)
  ...
  stepGenerateMeans = () => {
    const { extents, scales } = this.state;
    const means = this.initMeans(3, extents);
  ...
  stepFindCentroids = () => {
    ...
    for(let iteration = 0; iteration < 100; iteration++) {
      ...
      moveResult = this.moveMeans(means, assignments, trainingData, extents);

      if (moveResult.moved) {
        means = moveResult.nextMeans;
        this.renderMeans(means, scales, iteration);
        this.renderAssignmentLines(assignments, trainingData, means, iteration, false);
      } else {
        this.renderAssignmentLines(assignments, trainingData, means, iteration, true);
        break;
      }
    }
  }
    ...
    return (
      <div className="knn" style={{ marginLeft: 20 }}>
        ...
        <svg className="container" width={width} height={height}>
          ...
          <g className="assignments-plot-area" transform={`translate(${pad} ${pad})`}/>
          <g className="means-plot-area" transform={`translate(${pad} ${pad})`}/>

        </svg>

        <br/>
        <div>
          <button className="ui button" onClick={this.stepPlotTraining} style={{ width: 250, textAlign: 'left' }}>1. Plot random data</button>
        </div>
        <br/>
        <div>
          <button className="ui button" onClick={this.stepGenerateMeans} style={{ width: 250, textAlign: 'left' }}>2. Plot random mean points</button>
        </div>
        <br/>
        <div>
          <button className="ui button" onClick={this.stepFindCentroids} style={{ width: 250, textAlign: 'left' }}>3. Iterate centeroids</button>
        </div>
